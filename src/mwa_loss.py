...
class MWALoss(torch.nn.Module):
    ...
    def forward(self,
        ...
        ref_lens = torch.tensor([len(r) for r in ref_texts], device=device)

        wacs = ref_lens[nbest.shape.row_ids(1)] - wers
        wacs = torch.nn.functional.relu(wacs) # bound to 0
        wacs = wacs / ref_lens[nbest.shape.row_ids(1)] # normalize
        ...
        prob_normalized_raw = ragged_path_logp.normalize(use_log=True).values.exp()

        prob_normalized = prob_normalized_raw * wacs
        ...
        return loss, prob_normalized_raw, wacs
