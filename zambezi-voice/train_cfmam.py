...
class ASR(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.VALID or stage == sb.Stage.TEST or not self.seeding:
            ...
            if self.seeding:
                nbest = k2.nbest.Nbest.from_lattice(lattice, num_paths=self.hparams.num_paths, nbest_scale=self.hparams.nbest_scale)
        ...
        if not self.seeding:
            ...
            if len(char_labels) == 0:
                ...
                paths = {"cfm_rescoring": sbk2.lattice_decoder.one_best_decoding(lattice)}
        ...
        if (stage == sb.Stage.TEST or stage == sb.Stage.VALID) and not self.seeding:
            ...
            nbest.fsa.scores[nbest.fsa.labels > 0] = exact_log_p
            paths = {"cfm_rescoring":nbest.top_k(1).fsa}
...
def dataio_prepare(hparams):
    ...
    train_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=f"{hparams['output_folder']}/train.csv",
    ...
    valid_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=f"{hparams['output_folder']}/dev.csv",
        replacements={"data_root": data_folder},
    )
    valid_data = valid_data.filtered_sorted(sort_key="duration")

    test_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=f"{hparams['output_folder']}/test.csv",
        replacements={"data_root": data_folder},
    )
...
if __name__ == "__main__":
    ...
    from zambezivoice_prepare import prepare_zambezivoice, get_vocab  # noqa

    # multi-gpu (ddp) save data preparation
    run_on_main(
        prepare_zambezivoice,
        kwargs={
            "data_folder": hparams["data_folder"],
    ...
    get_vocab(
        csv_path=f"{hparams['output_folder']}/train.csv",
        dest_path=f"{hparams['vocab_file']}",
    )

    # here we create the datasets objects as well as tokenization and encoding
    train_data, valid_data, test_dataset = dataio_prepare(hparams)
    ...
    wer_dir = os.path.join(hparams["output_wer_folder"], "metric_test")
